  ...
  return (
    <>
      ...
      <div className={`fixed inset-0 z-40 xl:hidden ${open ? '' : 'pointer-events-none'}`} aria-hidden={!open}>
        <div className={`absolute inset-0 bg-slate-950/60 transition-opacity ${open ? 'opacity-100' : 'opacity-0'}`} onClick={onClose} />
        <aside className={`fixed left-0 top-0 z-50 h-full w-72 overflow-auto bg-slate-950/95 p-5 shadow-2xl transition-transform duration-300 ${open ? 'translate-x-0' : '-translate-x-full'}`}>
          <div className="mb-6 flex items-center justify-between">
            <div>
              <p className="text-sm font-semibold uppercase tracking-[0.28em] text-emerald-300">Workspace</p>
              <h2 className="text-2xl font-extrabold text-white">Admin Console</h2>
            </div>
            <button onClick={onClose} className="inline-flex items-center justify-center rounded-full bg-white/5 p-2 text-slate-100 transition hover:bg-white/10">
              <X size={18} />
            </button>
          </div>
          <nav className="space-y-2">
